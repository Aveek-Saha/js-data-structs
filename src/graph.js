...
export default function Graph(directed = false, weighted = false) {
    ...
    return {
        ...
        addEdge(value1, value2, weight = 1) {
            if (this.getNode(value1) === undefined) this.addNode(value1);
            if (this.getNode(value2) === undefined) this.addNode(value2);
        ...
        DFS(startValue) {
            ...
            function visit(currnode, visited) {
                ...
                currnode.adjacentNodes.forEach((node) => {
                    if (!visited[node.value]) visit(node, visited);
                });
            }
            return order;
        },
            ...
            for (let i = 0; i < matrix.length; i++) {
                if (!directed) {
                    for (let j = i + 1; j < matrix[i].length; j++) {
                        if (matrix[i][j] != 0) {
                            if (!this.weighted) this.addEdge(i, j);
                            else this.addEdge(i, j, matrix[i][j]);
                        }
                    }
                } else {
        ...
        topologicalSort() {
            ...
            nodes.forEach((node) => {
                if (!visited[node.value]) topSort(node);
            });
            ...
            for (let i = 0; i < n; i++) {
                ...
                nodes.forEach((node) => {
                    ...
                    for (let i = 0; i < edges.length; i++) {
                        var edge = edges[i];
                        if (
                            (edge[0].value == u.value &&
                                edge[1].value == node.value) ||
                            (edge[1].value == u.value &&
                                edge[2].value == node.value)
                        ) {
                            w = edge[2];
                            break;
                        }
                    ...
                    if (
                        !set[node.value] &&
                        w != 0 &&
                        distance[u.value] != Number.MAX_SAFE_INTEGER &&
        ...
        findEdgeWeight(u, v) {
            function match(edge) {
                return (
                    (edge[0].value == u && edge[1].value == v) ||
                    (edge[1].value == u && edge[0].value == v)
                );
            }
