...
return childNode}}}
exports.addBinarySearchNode=addBinarySearchNode;let order={pre:"pre",post:"post",in:"in"};function BinarySearchTree(rootValue){let root=addBinarySearchNode(rootValue);return{root,display(order){let result="";if(order=="post"){postOrder(root)}else if(order=="pre"){preOrder(root)}else{inOrder(root)}
function inOrder(node){if(node!=null){inOrder(node.left);result+=node.value+" ";inOrder(node.right)}
else return}
function postOrder(node){if(node!=null){postOrder(node.left);postOrder(node.right);result+=node.value+" "}
else return}
function preOrder(node){if(node!=null){result+=node.value+" ";postOrder(node.left);postOrder(node.right)}
else return}
return result},search(value){look(root)
...
exports.Queue=Queue;function PriorityQueue(){let queue=[];return{queue,enqueue(value,key){queue.push({value,key})
HeapSort(queue)},dequeue(){return queue.shift()},peek(){return queue[0].value},isEmpty(){return queue.length!=0?!1:!0}}}
...
exports.createGraphNode=createGraphNode;function Graph(directed=!1,weighted=!1){let nodes=[];let edges=[];return{directed,weighted,nodes,edges,addNode(value){nodes.push(createGraphNode(value))},getNode(value){return nodes.find(node=>node.value===value)},addEdge(value1,value2,weight=1){if(this.getNode(value1)===undefined)
this.addNode(value1);if(this.getNode(value2)===undefined)
this.addNode(value2);let node1=this.getNode(value1);let node2=this.getNode(value2);node1.addAdjacent(node2);edges.push([node1,node2,weight]);if(!this.directed){node2.addAdjacent(node1);edges.push([node2,node1,weight])}},BFS(startValue){let order=[];let startNode=this.getNode(startValue);let visited={};nodes.forEach(node=>{visited[node.value]=!1});visited[startNode.value]=!0;let q=Queue();q.enqueue(startNode);while(!q.isEmpty()){let currentNode=q.dequeue()
order.push(currentNode.value);currentNode.adjacentNodes.forEach(node=>{if(!visited[node.value]){q.enqueue(node);visited[node.value]=!0}})}
...
visit(node,visited)})}
return order},fromAdjMatrix(matrix){if(matrix.length!=matrix[0].length)
return null;for(let i=0;i<matrix.length;i++){if(!directed){for(let j=i+1;j<matrix[i].length;j++){if(matrix[i][j]!=0){if(!this.weighted)
this.addEdge(i,j);else this.addEdge(i,j,matrix[i][j])}}}
else{for(let j=0;j<matrix[i].length;j++){if(matrix[i][j]!=0){if(!this.weighted)
this.addEdge(i,j);else this.addEdge(i,j,matrix[i][j])}}}}},topologicalSort(){if(!this.directed)
...
return order.reverse()},dijkstra(start){let n=nodes.length;let distance={};let set={};nodes.forEach(node=>{distance[node.value]=Number.MAX_SAFE_INTEGER;set[node.value]=!1});console.log(edges.length);distance[this.getNode(start).value]=0;for(let i=0;i<n;i++){let u=minDistance(distance,set);set[u.value]=!0;nodes.forEach(node=>{let w=0;for(const edge of edges){if((edge[0].value==u.value&&edge[1].value==node.value)||(edge[1].value==u.value&&edge[2].value==node.value)){w=edge[2];break}}
if(!set[node.value]&&w!=0&&distance[u.value]!=Number.MAX_SAFE_INTEGER&&distance[u.value]+w<distance[node.value])
distance[node.value]=distance[u.value]+w})}
return distance;function minDistance(dist,set){let minDis=Number.MAX_SAFE_INTEGER;let minIndex=-1;nodes.forEach(node=>{if(set[node.value]==!1&&dist[node.value]<=minDis){minDis=dist[node.value];minIndex=node}});return minIndex}},findEdgeWeight(u,v){edges.forEach(edge=>{if((edge[0].value==u&&edge[1].value==v)||(edge[1].value==u&&edge[2].value==v))
return edge[2]})}}}
...
return result}
exports.powerSet=powerSet;function permutations(array){let n=array.length;let result="";let output=[];heaps_algorithm(array,n);function heaps_algorithm(arr,n){if(n==1){result+=arr.join("")+" ";return}
for(let i=0;i<n;i++){heaps_algorithm(arr,n-1);if(n%2==0){let x=array[i];array[i]=array[n-1];array[n-1]=x}
else{let x=array[0];array[0]=array[n-1];array[n-1]=x}}}
let temp=result.split(" ");let final=temp.slice(0,temp.length-1);final.forEach(ele=>{output.push(ele.split(""))});return output}
exports.permutations=permutations;function createNode(value){return{value,next:null}}
...
this.tail.next=node;this.tail=node;this.length++;return node},insertAt(index,value){if(index<0||index>this.length)
return null;if(index==this.length){let temp=this.push(value);return temp}
let node=createNode(value);if(index==0){node.next=this.head;this.head=node;this.length++;return node}
let curr=this.head;let i=0;while(index-1!=i){curr=curr.next;i++}
node.next=curr.next;curr.next=node;this.length++;return node},pop(){let node=this.tail;if(this.tail==this.head){this.head=null;this.tail=null;this.length--;return node}
...
curr.next=curr.next.next;this.length--;return del},display(){let curr=this.head;let result="";while(curr.next!=null){result+=curr.value.toString()+" ➞ ";curr=curr.next}
...
exports.HeapSort=HeapSort;function zSearch(text,pattern){let str=pattern+"$"+text;let len=str.length;let patLen=pattern.length;let result=[];let z=createZarray(str);for(let i=0;i<len;i++){if(z[i]==patLen)
result.push(i-patLen-1)}
function createZarray(string){let n=string.length;let arr=Array(n);arr[0]=0;let l=0,r=0;for(let i=1;i<n;i++){if(i>r){l=i;r=i;while(r<n&&string[r-l]==string[r]){r++}
arr[i]=r-l;r--}
else{let k=i-l;if(arr[k]<r-i+1)
arr[i]=arr[k];else{l=i;while(r<n&&string[r-l]==string[r]){r++}
arr[i]=r-l;r--}}}
return arr}
return result}
exports.zSearch=zSearch;function Nqueens(n){let board=Array.from(Array(n),()=>Array.from(Array(n),()=>0));solve(0);function isSafe(row,col){for(i=0;i<col;i++){if(board[row][i]==1)
return!1}
for(i=row,j=col;i>=0&&j>=0;i--,j--){if(board[i][j]==1)
return!1}
for(i=row,j=col;j>=0&&i<n;i++,j--){if(board[i][j]==1)
return!1}
return!0}
function solve(col){if(col>=n)
return!0;for(let i=0;i<n;i++){if(isSafe(i,col)){board[i][col]=1;if(solve(col+1))
return!0;board[i][col]=0}}
return!1}
return board}
